...
RUN --mount=type=bind,target=/azure-cli.rpm,source=./docker-temp/azure-cli.rpm \
    ...
    # Keep versionCheck.json to prevent az from checking the latest version on GitHub
    az version && cd ~/.azure && ls | grep -v commandIndex.json | grep -v versionCheck.json | xargs rm -rf && \
